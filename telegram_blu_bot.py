...
# --- DATABASE FLASH SALE CLAIMS (1X PER USER PER BULAN/PERIODE) ---
...
def has_user_claimed_flash_sale(user_id: int) -> bool:
    ...
    # Format key unik per tanggal (misal: "7814906855_2026-07-25")
...
def add_history_record(amount: int, pkg_name: str, token: str, chat_id: int):
...
def get_package_price_for_user(user_id: int, base_price: int) -> tuple[int, bool]:
    """Mengembalikan tuple: (harga_akhir, is_flash_sale_applied)"""
    if is_flash_sale_active() and not has_user_claimed_flash_sale(user_id):
        disc_price = int(base_price * (1 - FLASH_SALE_DISCOUNT))
        return disc_price, True
...
# --- EMAIL LISTENER (BLU BCA IMAP) ---
def check_gmail_for_blu_transfers(app, loop):
    print("📧 Service Email Listener (blu BCA) Aktif & Monitoring...")
    while True:
        try:
                        ...
                        if isinstance(response_part, tuple):
                            ...
                            for amt in detected_amounts:
                                ...
                                if inv:
                                    ...
                                    add_history_record(package_cost, inv["pkg_name"], token, chat_id)
...
def get_main_keyboard(user_id: int):
    ...
    buttons.append([InlineKeyboardButton(f"💰 Saldo Saya: {fmt_idr(user_bal)}", callback_data="my_balance")])
...
async def myid_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        ...
        f"• *FLASH SALE (TGL KEMBAR & TGL 25)*: {'`Sudah Diklaim Hari Ini`' if has_claimed else ('`Tersedia (Aktif Hari Ini)`' if is_flash_sale_active() else '`Tersedia Tiap Tgl Kembar & Tgl 25`')}"
    )
...
async def render_admin_panel(update_or_query, user_id: int):
    ...
    text = (
        ...
        f"🛠️ *Perintah Admin Manual*:\n"
        f"• `/gen 100` / `/gen 210` / `/gen 460` / `/gen 1250` (Buat voucher gratis)"
    )
...
async def riwayat_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await render_user_history(update, update.effective_user.id)
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if pkg_key in PACKAGES:
        ...
        item_price, is_flash = get_package_price_for_user(user_id, pkg_info["base_price"])
        user_bal = get_user_balance(user_id)
        ...
        if user_bal >= item_price:
            ...
            add_history_record(item_price, pkg_info["name"], token, user_id)

            instant_text = (
                f"🎉 *PEMBAYARAN SUKSES VIA SALDO BOT!*\n\n"
                f"📦 *Paket*: {pkg_info['name']}\n"
                f"💰 *Harga*: {fmt_idr(item_price)} {'*(Diskon 40% Applied)*' if is_flash else ''}\n"
            ...
            keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Kembali ke Menu Utama", callback_data="start_menu")]])
            await query.edit_message_text(text=instant_text, parse_mode="Markdown", reply_markup=keyboard)
            return
        ...
        # Angka murni untuk di-copy (tanpa titik, koma, atau Rp)
        clean_amount = str(exact_amount) 
        # Format tampilan cantik dengan Rp dan ,00
        ...
        await query.edit_message_text(text=invoice_text, parse_mode="Markdown", reply_markup=keyboard)
        ...
        cancel_text = "❌ *PESANAN DIBATALKAN.*\n\n(Catatan: Jika Anda sebenarnya sudah terlanjur transfer, sistem blu BCA kami akan TETAP otomatis mendeteksi, mengirim voucher, dan menambah sisa saldo Anda)."
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Kembali ke Menu Utama", callback_data="start_menu")]])
        ...
        bal_text = (
            ...
            f"📌 *Informasi Saldo*:\n"
            f"• Saldo otomatis bertambah jika Anda transfer lebih dari harga paket (misal karena batas minimal transfer bank Rp 10.000).\n"
...
async def post_init(application: Application) -> None:
    bot_commands = [
        ...
        BotCommand("myid", "👤 Cek ID Telegram & Saldo"),
        BotCommand("gen", "🔑 Generate Voucher Manual (Khusus Admin)")
    ]
...
def main():
    ...
    print("🤖 Bot Telegram Auto-Payment (blu BCA Email Detector) Aktif...")
    app.run_polling()
